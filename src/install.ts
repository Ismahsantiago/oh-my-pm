...
async function writeOhMyPmReference(targetDir: string): Promise<readonly string[]> {
...
async function installOpenCode(projectRoot: string): Promise<readonly string[]> {
  ...
  for (const p of await writeOhMyPmReference(projectRoot)) written.push(p)
  return written
}
...
async function installGlobalOpenCode(): Promise<readonly string[]> {
  ...
  for (const p of await writeOhMyPmReference(configRoot)) written.push(p)
  return written
}

async function installCursor(projectRoot: string): Promise<readonly string[]> {
  const root = await templateRoot()
  const written: string[] = []
  for (const agent of AGENTS) {
    const source = path.join(root, "opencode", "skills", agent, "SKILL.md")
...
async function copyCursorPluginConfig(targetRoot: string): Promise<string> {
...
function selectedOptionalPlatforms(options: InstallOptions): readonly Platform[] {
  if (options.all === true) return ["claude", "cursor", "openai", "generic"]
  const selected: Platform[] = []
  if (options.claude === true) selected.push("claude")
  if (options.cursor === true) selected.push("cursor")
